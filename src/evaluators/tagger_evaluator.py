    ...
    def report_results(self, sen_counter, counts, loss, file=sys.stderr):
        ...
        for i in range(last_index):
            for key in ('{}:in'.format(i), '{}:out'.format(i)):
                ...
                r = 1.0 * ncorrect / ngold
                print(
                    '%s vocab-%d (%.2f%%) - N_gold, TP, R: %d, %d,\t%.2f' %
                    ('In    ' if 'in' in key else 'Out-of', i,
                     (100.0 * ngold / total_ngold), ngold, ncorrect, 100 * r),
                    file=file)
...
class FMeasureEvaluatorForEachLength(object):
    ...
    def report_results(self, sen_counter, counts, loss, file=sys.stderr):
        ...
        max_l = max(
            set(counts.len2ngold.keys()) | set(counts.len2npred.keys()))
        for l in range(max_l + 1):
            if l not in counts.len2ngold and l not in counts.len2npred:
                continue
            ...
            print(
                'Len=%2d (%.2f%%) - N_gold, N_pred, TP, P, R, F: %d, %d, %d,\t%.2f, %.2f, %.2f'
                % (l, (100.0 * ngold / total_ngold), ngold, npred, ncorrect,
                   100 * p, 100 * r, 100 * f),
                file=file)
        ...
        if True:
            ...
            print(
                'Total          - N_gold, N_pred, TP, P, R, F: %d, %d, %d,\t%.2f, %.2f, %.2f'
                % (total_ngold, total_npred, total_ncorrect, 100 * p, 100 * r,
                   100 * f),
                file=file)
