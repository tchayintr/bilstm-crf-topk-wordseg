...
class Trainer(object):
    ...
    def setup_optimizer(self):
        # mparams = self.classifier.predictor.parameters()
    ...
    def run_eval_mode(self):
        ...
        time = datetime.now().strftime('%Y%m%d_%H%M')
        self.report('test {}'.format(res))
        self.report('Finish: {}'.format(time))
        self.log('Finish: {}'.format(time))

    def run_decode_mode(self):
        self.update_model(classifier=self.classifier, dic=self.dic)
        if self.args.output_data:
            output_data = '{}/{}'.format(constants.DECODE_DIR,
